(function ($) {
  ...
  Drupal.behaviors.commerce_backoffice_user_orders = {
    attach: function (context,settings) {
      var useroders;
      useroders=$('.view-commerce-backoffice-user-orders',context);
      $('tr', useroders).each(function(){
        var status;
        status=$('td.views-field-status', $(this)).text().trim();
        if(status==='结账： 支付'){
          $('a.immpayment', $(this)).show();
        }else{
          $('a.immpayment', $(this)).hide();
        }
      });
    }
  };
